...
function calcAnswerFromOutcome(opponentAnswer: OpponentAnswer, outcome: Outcome): OpponentAnswer {
  if (outcome === 'Y') {
...
const score = guide.reduce((prev, round) => prev + roundScore(round[0], round[1]), 0);
const score2 = guide.reduce((prev, round) => prev + roundScoreOutcomes(round[0], round[1]), 0);
